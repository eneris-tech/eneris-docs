...
/**
 * Verify spotlight — extracts each spotlight region as a standalone cropped sub-image
 * with padding so Claude can confirm the region contains the intended UI element
 * BEFORE running the spotlight tool.
 *
 * Usage:
 *   bun run verify-spotlight.ts -i input.png --region "100,200,400,300"
 *   bun run verify-spotlight.ts -i input.png --region "100,200,400,300" --region "600,50,200,150" --padding 30
 *
 * Options:
 *   --input / -i      Source image (same image you'd pass to spotlight)
...
if (!values.input || !values.region?.length) {
  console.error(
    'Usage: bun run verify-spotlight.ts -i <input> --region "left,top,width,height" [--region ...] [--padding 20]'
  );
  process.exit(1);
}
...
try {
  ...
  for (let i = 0; i < regions.length; i++) {
    ...
    const outPath = `/tmp/verify-region-${i + 1}.png`;
