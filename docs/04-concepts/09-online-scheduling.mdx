...
- **Notify additional team members**. The team members who get emailed when a request arrives. The assigned inspector is always emailed, even if they are not selected here.
...
- **Time zone**. Used to compute available slots and shown to the client on the public page.
...
- **Minimum notice (hours)**. Minimum advance notice for a booking. Defaults to 8 hours.
- **How far in advance clients can book (days)**. How far in the future bookings are allowed (1 to 180 days).
...
- **New request submitted** (public page submission):
  ...
  - Anyone listed under **Notify additional team members** on the booking page is also emailed.
